...
//        boolean negative = false;
   ...
   /*  this function accepts 4 bytes and returns an integer
    */
...
//       boolean negative = false;
     ...
     * The bytes <code>buf[i..i+length-1]</code> are used.
     */
    public static final String intArrayToHexString(int[] buf, int i, int length)
    {
        StringBuffer sb = new StringBuffer(length*2);
        for (int j=i; j<i+length; j++) {
            sb.append(NIBBLE[(buf[j]>>>4)&15]);
     ...
     * Convert a byte array to a string of hexadecimal digits.
     */
    public static final String intArrayToHexString(int[] buf)
    {
        return intArrayToHexString(buf, 0, buf.length);
    }
    ...
    public static byte hexDigitToByte(char hDigit)
    {
        ...
        if(hDigit >= 'a' &&  hDigit <= 'g')
            return (byte)(hDigit-('a'-10));

        throw new IllegalArgumentException("Invalid hex digit " + hDigit);
    }
     ...
     */
    public static byte[] hexStringToBytes(String hex1)
    {
        String hex = "";
