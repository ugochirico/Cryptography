package com.ugochirico.crypt.rijndael;
...
public class Rijndael
{
    ...
    public static final int BLOCK_SIZE_224 = 224;
    public static final int BLOCK_SIZE_160 = 160;
    public static final int BLOCK_SIZE_192 = 192;
        ...
        byte[]      key)
    {
        ...
        case 256:
            ...
            break;
        default :
            throw new IllegalArgumentException("Key length not 128/160/192/224/256 bits.: " + keyBits);
        }
    ...
    /**
     * Expand a user-supplied key material into a safe session key having the right size.
     *
     * @param key        The key material to use for generating a safe key.
     * @param blockSize  The block size in bytes of this Rijndael.
     * @exception  InvalidKeyException  If the key is invalid.
     */
    public static synchronized byte[] makeSafeKey (byte[] k, int blockSize)
    {
        ByteArrayOutputStream outs = new ByteArrayOutputStream(blockSize);

        int nDiff = (blockSize / 8) - k.length;
        try
        {
            outs.write(k);
        ...
        int outOff)
    {
        if (workingKey == null)
        {
            throw new IllegalStateException("AESECB engine not initialised");
        }
    ...
    public byte[] encryptECB(byte[] plaintext, int nOffset)
    {
        ...
        {
            encryptBlock(plaintext, nOffset, encblock, 0);
    ...
    public byte[] decryptECB(byte[] ciphertext, int nOffset)
    {
        ...
        try
        {
            ...
            {
                decryptBlock(ciphertext, nOffset, block, 0);
...
//            System.out.println(Encoder.bytesToHexString(block));
            ...
            encryptBlock(xoredblock, 0, encblock, 0);
...
//            System.out.println(Encoder.bytesToHexString(encblock));
    ...
    public byte[] decryptCBC(byte[] ciphertext, int nOffset, byte[] iv)
    {
        ...
        while(nOffset < ciphertext.length)
        {
            decryptBlock(ciphertext, nOffset, xoredblock, 0);
...
//            System.out.println(Encoder.bytesToHexString(block));
    ...
    private byte[] pad(byte[] plaintext, int nOffset)
    {
        ...
        // check padding
        int nPadLen = (getBlockSize() - ((plaintext.length - nOffset) % getBlockSize())) - 1;
...
//        System.out.println(Encoder.bytesToHexString(plainText));
     ...
     * @return The ciphertext generated using the session key.
     */
    public static byte[] encryptECB(byte[] plaintext, int nOffset, byte[] key, int nBlockLen)
    {
        Rijndael rij = new Rijndael(nBlockLen);
        rij.setKey(makeSafeKey(key, nBlockLen));

        return rij.encryptECB(plaintext, nOffset);
    }
     ...
     * @return The plaintext generated from ciphertext using the session key.
     */
    public static byte[] decryptECB(byte[] ciphertext, int nOffset, byte[] key, int nBlockLen)
    {
        Rijndael rij = new Rijndael(nBlockLen);
        rij.setKey(makeSafeKey(key, nBlockLen));

        return rij.decryptECB(ciphertext, nOffset);
    }

    public static byte[] encryptCBC(byte[] plaintext, int nOffset, byte[] key, byte[] iv, int nBlockLen)
    {
        Rijndael rij = new Rijndael(nBlockLen);
        rij.setKey(makeSafeKey(key, nBlockLen));

        return rij.encryptCBC(plaintext, nOffset, makeSafeKey(iv, nBlockLen));
    }
     ...
     * @return The plaintext generated from ciphertext using the session key.
     */
    public static byte[] decryptCBC(byte[] ciphertext, int nOffset, byte[] key, byte[] iv, int nBlockLen)
    {
        Rijndael rij = new Rijndael(nBlockLen);
        rij.setKey(makeSafeKey(key, nBlockLen));
        return rij.decryptCBC(ciphertext, nOffset, makeSafeKey(iv, nBlockLen));
    }

    public class RijndaelException extends RuntimeException
    {
        RijndaelException()
        {
            super("Bad key");
...
//               
//        byte[] enc = encryptCBC(msg.getBytes(), 0, key.getBytes(), iv.getBytes(), Rijndael.BLOCK_SIZE_192);
//        System.out.println(Encoder.bytesToHexString(enc));
//        
//        byte[] dec = decryptCBC(enc, 0, key.getBytes(), iv.getBytes(), Rijndael.BLOCK_SIZE_192);
//        System.out.println(new String(dec));
//
//        enc = encryptECB(msg.getBytes(), 0, key.getBytes(), Rijndael.BLOCK_SIZE_192);
//        System.out.println(Encoder.bytesToHexString(enc));
//        
//        dec = decryptECB(enc, 0, key.getBytes(), Rijndael.BLOCK_SIZE_192);
//        System.out.println(new String(dec));
//
//    }
}
