package com.ugochirico.crypt.AES;
...
/**
 * an implementation of the AESECB (Rijndael), from FIPS-197.
 * <p>
...
private static final int[] Tinv0 =
    ...
    */
     ...
     * AESECB specified a fixed block size of 128 bits and key sizes 128/192/256 bits
...
//        }
        ...
        byte[] key, int keySize)
    {
        ...
        {
            throw new IllegalArgumentException("Invalid key size (128/192/256)");
        }
        ...
        {            
            ...
            outs.write(key, 0, key.length);
        }
        
        WorkingKey = generateWorkingKey(outs.toByteArray());
        //this.forEncryption = forEncryption;
    }
...
//            }
     ...
     */
    public byte[] encrypt(byte[] plaintext, int nOffset)
    {
        ByteArrayOutputStream outsPlainText = new ByteArrayOutputStream();

        // check padding
        int nPadLen = BLOCK_SIZE - ((plaintext.length - nOffset) % BLOCK_SIZE);
        outsPlainText.write(nPadLen);        
        for(int i = 0; i < nPadLen - 1; i++)
        {
            outsPlainText.write((char)0xFF);
        }
        ...
        // encrypt
        plaintext = outsPlainText.toByteArray();
        byte[] block = new byte[BLOCK_SIZE];
        
        ByteArrayOutputStream outsCipherText = new ByteArrayOutputStream();
        while(nOffset < plaintext.length)
        {
            encryptBlock(plaintext, nOffset, block, 0);
            outsCipherText.write(block, 0, BLOCK_SIZE);
            nOffset += BLOCK_SIZE;
        }
        
        //nPadLen = outsCipherText.toByteArray().length % blockSize;
     ...
     */
    public byte[] decrypt(byte[] ciphertext, int nOffset)
    {
        ByteArrayOutputStream outs = new ByteArrayOutputStream();
        byte[] block = new byte[BLOCK_SIZE];
        ...
        while(nOffset < ciphertext.length)
        {
            ...
            outs.write(block, 0, BLOCK_SIZE);            
            nOffset += BLOCK_SIZE;
        }
        
        byte plainText[] = outs.toByteArray();
        // check padding
       // System.out.println("Padlen:"+ plainText[0]);
        int nPadLen = (((plainText[0] & 0xF0) >> 4) * 16) + (plainText[0] & 0x0F);
        
        //System.out.println("Padlen:"+ nPadLen);
        //System.out.println(Encoder.bytesToHexString(plainText));
        
        ByteArrayOutputStream outsPlainText = new ByteArrayOutputStream();
        outsPlainText.write(plainText, nPadLen, plainText.length - nPadLen);
