...
import com.ugochirico.crypt.ecc.DifferentFieldsException;
...
//    protected static final boolean DEBUG = false;
     ...
     */
    public GFp(BigInteger p) throws ArithmeticException {
...
//        }
     ...
     */
    public GFp(BigInteger p, BigInteger x) throws ArithmeticException
    {
        //this(p);
...
//        count++;
        q = p;
        v = x;
//        MIABO.m_waitForm.update(count + " gfp OK ",1);
    }
     ...
     */
    public GFp(BigInteger p, String val, int radix) throws NumberFormatException {
     ...
     * @exception   NumberFormatException   if val is not in appropriate format
     */
    public GFp(BigInteger p, String val) throws NumberFormatException {
        this(p, val, 16);
    }
     ...
     */
    public GFp(BigInteger p, Random rand) {
     ...
     */
    public GF randomize(Random rand)
    {
    ...
    public GF shiftRight(int n) {
        ...
        return (n <  0) ? shiftLeft(-n)
            ...
            : new GFp(q, v.multiply(BigInteger.ONE.shiftLeft(n).modInverse(q)).mod(q));
    }
    ...
    public GF sqrt() {
        ...
        if (q.testBit(1)) {
            GFp z = new GFp(q, v.modPow(q.shiftRight(2).add(BigInteger.ONE), q));
//            return z;
            // test solution:
        ...
        if (q.testBit(2)) {
            ...
            GFp z = new GFp(q, v.multiply(gamma).multiply(i.subtract(BigInteger.ONE)).mod(q));
//            return z;
            // test solution:
            return z.square().equals(this) ? z : null;
        }
        // case III: q mod 4 == q mod 8 == 1 (bad news: this takes very long to compute...)
        BigInteger k = q.add(BigInteger.ONE).shiftRight(1);
        for (BigInteger P = BigInteger.ONE; /*P.compareTo(q) < 0*/; P = P.add(BigInteger.ONE)) {            // compute the Lucas sequence elements V = V_{(q+1)/2} mod q and Q_0 = this^{(q-1)/4)} mod q:
...
//            return z;
            ...
            if (VQ0[1].compareTo(BigInteger.ONE) > 0 && VQ0[1].compareTo(q.subtract(BigInteger.ONE)) < 0) {                return null; // no square roots exist
            }
        }
    }
    ...
    public int sqrtCount() {
        BigInteger count = v.modPow(q.subtract(BigInteger.ONE).shiftRight(1), q).add(BigInteger.ONE).mod(q);
        /*
