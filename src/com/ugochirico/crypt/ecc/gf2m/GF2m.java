...
//        }
     ...
     */
    public GF2m(int m, BigInteger x) throws ArithmeticException {
        this(m);
...
//        }
     ...
     * @exception   ArithmeticException     if x is not in range 0 to 2<SUP>m</SUP> - 1 (DEBUG mode only)
     */
    public GF2m(int m, int[] x) throws ArithmeticException {
     ...
     */
    public GF2m(int m, String val, int radix) throws NumberFormatException {
     ...
     * @exception   NumberFormatException   if val is not in appropriate format
     */
    public GF2m(int m, String val) throws NumberFormatException {
        this(m, val, 16);
    }
     ...
     * @param   rand    cryptographically strong PRNG
     */
    public GF2m(GF2m x, Random rand) {
        this.m = x.m;
        this.q = new BigInteger(x.q.toByteArray());
    ...
    */
    ...
    public GF invert() throws ArithmeticException {
        ...
        int[] G = GF2mUtil.getIrredPoly(m).toIntArray(); int degG =  m;
    ...
    public GF shiftRight(int n) {
        if (n <  0) {
            ...
            short[] red = GF2mUtil.getIrredArray(m);
            BigInteger B = this.v;
            if (red.length == 1) {
                int k = red[0]; // reduction polynomial is x^m + x^k + 1
                BigInteger mask = BigInteger.ZERO.setBit(k).subtract(BigInteger.ONE);
                while (n > 0) {
                    int t;
                    if (!B.testBit(0) && (t = B.getLowestSetBit()) >= k) {
                        ...
                        if (t > n) {
                            t = n;
                            mask = BigInteger.ZERO.setBit(n).subtract(BigInteger.ONE);
                        }
                        /*
                ...
                BigInteger mask = BigInteger.ZERO.setBit(k1).subtract(BigInteger.ONE);
                while (n > 0) {
                    int t;
    ...
    public int trace() {
        ...
        short[] tab = GF2mUtil.getTraceTable(m);
        if (tab != null) {
            /*
    ...
    public GF quadSolve() {
        ...
        if ((m & 1) == 1) {
            ...
            if (tab != null) {
                ...
                t = new GF2m(m, BigInteger.ZERO.setBit(tab[0]));
            } else {
                // bad luck: must compute the desired element the slow way
                for (int i = 0; i < m; i++) {
                    t = new GF2m(m, BigInteger.ZERO.setBit(i));
                    if (t.trace() == 1) {
                        break;
                    }
                }
