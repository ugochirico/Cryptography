...
public final class BigInteger
{
    ...
    private static final void destructiveMulAdd(int[] x, int y, int z) {
    ...
    private static final byte[] randomBits(int numBits, Random rnd) {
	if (numBits < 0)
...
//     */
     ...
     */
    final int jacobiSymbol(int p, BigInteger n) {
        if (p == 0)
            return 0;
...
//    }
     ...
     */
    private static final BigInteger valueOf(int val[]) {
     ...
     */
    public final BigInteger add(BigInteger val) {
        int[] resultMag;
	if (val.signum == 0)
            return this;
	if (signum == 0)
     ...
     * @return <tt>this - val</tt>
     */
    public final BigInteger subtract(BigInteger val) {
        int[] resultMag;
	if (val.signum == 0)
            return this;
	if (signum == 0)
     ...
     * answer.
     */
    private static final int[] subtract(int[] big, int[] little) {
     ...
     * @return <tt>this * val</tt>
     */
    public final BigInteger multiply(BigInteger val) {
     ...
     */
    public final BigInteger divide(BigInteger val) {
     ...
     * @throws ArithmeticException <tt>val==0</tt>
     */
    public final BigInteger[] divideAndRemainder(BigInteger val) {
     ...
     * @throws ArithmeticException <tt>val==0</tt>
     */
    public final BigInteger remainder(BigInteger val) {
        MutableBigInteger q = new MutableBigInteger(),
                          r = new MutableBigInteger(),
                          a = new MutableBigInteger(this.mag),
                          b = new MutableBigInteger(val.mag);
     ...
     */
    public final BigInteger gcd(BigInteger val) {
        if (val.signum == 0)
	    return this.abs();
     ...
     */
    private static final int[] leftShift(int[] a, int len, int n) {
        int nInts = n >>> 5;
        int nBits = n&0x1F;
    ...
    static final void primitiveRightShift(int[] a, int len, int n) {
        int n2 = 32 - n;
    ...
    static final void primitiveLeftShift(int[] a, int len, int n) {
        if (len == 0 || n == 0)
            return;
     ...
     */
    private static final int bitLength(int[] val, int len) {
        if (len==0)
            return 0;
     ...
     * @see    #remainder
     */
    public final BigInteger mod(BigInteger m) {
	if (m.signum <= 0)
...
//        int[] base = (int[])mag.clone();
        ...
        System.arraycopy(mag, 0, base, 0, base.length);
...
//                    b = (int[])mult.clone();
                    ...
                    System.arraycopy(mult, 0, b, 0, b.length);
     ...
     */
    private static final int[] montReduce(int[] n, int[] mod, int mlen, int inv) {
        int c=0;
        int len = mlen;
        int offset=0;
     ...
     */
    private static final int intArrayCmpToLen(int[] arg1, int[] arg2, int len) {
	for (int i=0; i<len; i++) {
     ...
     */
    private static final int subN(int[] a, int[] b, int len) {
        long sum = 0;
     ...
     */
    static final int mulAdd(int[] out, int[] in, int offset, int len, int k) {
     ...
     * carry.
     */
    static final int addOne(int[] a, int offset, int mlen, int carry) {
     ...
     */
    public final BigInteger modInverse(BigInteger m) {
	if (m.signum != 1)
	    throw new ArithmeticException("BigInteger: modulus not positive");
     ...
     * @see #shiftRight
     */
    public final BigInteger shiftLeft(int n) {
        if (signum == 0)
            return ZERO;
        if (n==0)
            return this;
     ...
     * @see #shiftLeft
     */
    public final BigInteger shiftRight(int n) {
        if (n==0)
            return this;
        if (n<0)
            return shiftLeft(-n);
    ...
    final int[] javaIncrement(int[] val) {
//        boolean done = false;
        int lastSum = 0;
     ...
     * @return <tt>this &amp; val</tt>
     */
    public final BigInteger and(BigInteger val) {
     ...
     * @return <tt>this | val</tt>
     */
    public final BigInteger or(BigInteger val) {
	int[] result = new int[Math.max(intLength(), val.intLength())];
	for (int i=0; i<result.length; i++)
	    result[i] = (int) (getInt(result.length-i-1)
				| val.getInt(result.length-i-1));
     ...
     * @return <tt>this ^ val</tt>
     */
    public final BigInteger xor(BigInteger val) {
	int[] result = new int[Math.max(intLength(), val.intLength())];
	for (int i=0; i<result.length; i++)
	    result[i] = (int) (getInt(result.length-i-1)
				^ val.getInt(result.length-i-1));
     ...
     * @return <tt>this &amp; ~val</tt>
     */
    public final BigInteger andNot(BigInteger val) {
	int[] result = new int[Math.max(intLength(), val.intLength())];
	for (int i=0; i<result.length; i++)
	    result[i] = (int) (getInt(result.length-i-1)
				& ~val.getInt(result.length-i-1));
    ...
    static final int bitCnt(int val) {
    ...
    static final int trailingZeroCnt(int val) {
     ...
     */
    public final int compareTo(BigInteger val) {
     ...
     */
    private static final int intArrayCmp(int[] arg1, int[] arg2) {
     ...
     */
    public final BigInteger min(BigInteger val) {
     ...
     *         <tt>val</tt>.  If they are equal, either may be returned.
     */
    public final BigInteger max(BigInteger val) {
	return (compareTo(val)>0 ? this : val);
    }
...
//    }
     ...
     */
    private static final int[] stripLeadingZeroInts(int val[]) {
     ...
     */
    private static final int[] trustedStripLeadingZeroInts(int val[]) {
        //int byteLength = val.length;
	int keep;
     ...
     * Returns a copy of the input array stripped of any leading zero bytes.
     */
    private static final int[] stripLeadingZeroBytes(byte a[]) {
     ...
     */
    private static final int[] makePositive(byte a[]) {
	int keep, k;
        int byteLength = a.length;
     ...
     */
    private static final int[] makePositive(int a[]) {
	int keep, j;
    ...
    private static final BigInteger longRadix[] = {null, null,
...
//     {
//         //BigInteger.setMaxDigitsLen(BigInteger.NN_ECC_512_BYTE_LEN);
//
//         BigInteger bb1 = new BigInteger("0A0B0C", 16);
//         System.out.println(bb1.toString(16));
//
//         BigInteger bb2 = new BigInteger("01", 16);
//         System.out.println(bb2.toString(16));
//
//         System.out.println(bb1.subtract(bb2).toString(16));
//
//         System.out.println(bb1.subtract(ZERO).toString(16));
//
//         BigInteger bn = ZERO.subtract(bb2);
//         System.out.println(bn.toString(16));
//         System.out.println(Encoder.bytesToHexString(bn.toByteArray()));
//
//         BigInteger bn1 = new BigInteger(bn.toByteArray());
//
//         System.out.println(bn1.toString(16));
//         System.out.println(Encoder.bytesToHexString(bn1.toByteArray()));
//     }
}
